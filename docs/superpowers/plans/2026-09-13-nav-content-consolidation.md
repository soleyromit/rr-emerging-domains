...
**Files:**
- Modify: `app/domains/[slug]/dissect/page.tsx:198-200` — the "1 of 6 dissection questions have partial-or-better coverage" stat currently uses `Takeaway status="warning"`. Change it to `status="accent"` (informational, not cautionary — nothing is wrong here, the page is just incomplete) so it no longer shares a color with the genuine quarantine notice below it on the same page.
