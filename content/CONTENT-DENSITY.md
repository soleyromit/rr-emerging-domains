...
| `accreditation/*.yaml` `licensure_or_gme_layer[].relevance_to_product` | 921 | 1,500 | **new 2026-09-11** — this field had no row at all, and a Task 2.3 review found the consequence: `acpe.yaml`'s State Boards entry reached 1,041 chars (2.2x the next-longest entry in its own file) while the checker stayed green, because nothing reached it. Soft is p75 of the 52-entry corpus across all accreditation files; hard sits just above the observed max (1,424, `coa.yaml`'s NBCRNA entry) rather than at a true p95, since these entries have never had an individual spot-check pass — so the row guards drift without retroactively failing unreviewed content. Tighten the hard ceiling once that pass happens. |
...
| `domains/*.yaml` `closest_analog.similarity` | 160 | 260 | **new 2026-09-13** with the optional `closest_analog:` block — one sentence naming both disciplines and the structural axis they share. Same numbers as `lenses/competitor-differentiation.yaml`'s `differentiators[].claim`, the closest analog, and for the identical reason that row gives: it sits above the 120/220 headline register because the sentence has to name both parties and the axis before it says anything. |
| `domains/*.yaml` `closest_analog.reuse_note` | 200 | 350 | **new 2026-09-13** — what can be scaffolded from the analog rather than rebuilt, never the argument for it (that belongs in `similarity` and its `source`). Same numbers as `lenses/competitor-differentiation.yaml`'s `differentiators[].counter_move`, the closest analog: a stated move, one sentence with a subject and a mechanism. |
...
| `archetypes/*.yaml` `name` | 120 | 220 | **new 2026-09-13** with the family — the scannable row label for one university segment, same register and numbers as `lenses/feature-comparison-matrix.yaml`'s `cells[].capability`: a reader skims down these, they are not the argument. |
| `archetypes/*.yaml` `description` | 300 | 600 | **new 2026-09-13** — one paragraph saying what the segment is and why it groups together, mirroring `lenses/product-gaps.yaml`'s `gaps[].detail` exactly: a sourced paragraph behind a plain-language headline. A segment that needs more than this is two segments. |
| `archetypes/*.yaml` `identifying_traits[]` | 150 | 300 | **new 2026-09-13** — **per entry, not per field**: one independently checkable criterion for "is this university in this segment?", the same numbers as `ratings[].evidence_note`. A trait that needs a paragraph is really two traits (the decompose rule below). |
| `archetypes/*.yaml` `gtm_approach` | 200 | 350 | **new 2026-09-13** — the stated move for the segment, never its justification (that is `description` and its source). Same numbers and same job as `lenses/competitor-differentiation.yaml`'s `differentiators[].counter_move`. **The whole family ships zero entries today** — `content/archetypes/` holds only `_TEMPLATE.yaml`, because the 2026-09-12 planning session assigned university archetypes as strategy work that has not been done — so all four rows gate the first real entry on arrival rather than being retrofitted onto content already written. |
| `gtm/research-pipeline.yaml` `workstreams[].objective` | 200 | 350 | **new 2026-09-11** with the `gtm/` family — what one workstream is trying to find out, in a line or two. Same soft target as `dissection/*.yaml`'s `questions[].gap_note`, whose hard ceiling is 400 — this one stops at 350, because a gap note carries two clauses (what is absent, and why) while an objective carries one. Ceilinged in that register rather than as a paragraph field for the same reason that row gives: the file holds no findings, so anything longer than "what we're trying to learn and why" is already the answer, which belongs in the file named in `produces:`. |
| `prism/capability-map.yaml` `*.roadmap.scope_note` (pillar or feature) | 200 | 400 | **new 2026-09-11** with the optional `roadmap:` sub-object — what a dated release actually contains, never why it matters: the argument already has a home in the same entry's `why_it_matters` (1,200/2,200), and a scope note that starts arguing is that field with a different name. Sized like `dissection/*.yaml`'s `questions[].gap_note` rather than a one-line evidence note, because it carries two clauses (what ships, and what explicitly does not). **Enforced as of 2026-09-11** (it was documentation-only when first written) on both attachment points, pillar and feature — no entry populates the field yet, so it gates the first one on arrival rather than being retrofitted. |
| `gtm/*.yaml` `blocked_by` / `notes` | 150 | 300 | **new 2026-09-11** with the `gtm/` family — one line saying what is actually blocking a workstream, or one line of tracking note on a collateral asset. Same numbers as `ratings[].evidence_note` and `market/programs/*.yaml` `programs[].notes`, the closest analogs: a one-clause status label on a tracker row, not an argument. Covered by one row because both fields are the same shape of short free text on a row that asserts nothing. |
| `gtm/collateral.yaml` `assets[].title` | 150 | 300 | **new 2026-09-11**, documented in `collateral.yaml`'s own row-shape comment when the family landed and written into this table 2026-09-11 so the two agree — the name of someone else's artifact, not a description of it. |

Every row above is now mechanically enforced. The four families added on 2026-09-11
(`lenses/product-gaps.yaml`, `lenses/competitor-differentiation.yaml`,
`lenses/feature-comparison-matrix.yaml`, `gtm/*.yaml`) spent their first day as
documentation-only rows, which is exactly the state this document's own opening incident
is about; `content/gtm/` was additionally absent from `check_content_density.py`'s
directory enumeration, so its ceilings could not have fired even had they been declared.
Both gaps closed 2026-09-11. If you add a row here, wire it in the same commit.
