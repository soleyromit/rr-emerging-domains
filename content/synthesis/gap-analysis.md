...
|---|---|
| Clinical **site** (executed, unexpired affiliation agreement) | ACPE 3.5.b (*Configure since the 2026-09-10 correction, was Gap*), COCA 6.9, LCME 1.4, CODA 4-6 |
...
The three files used to disagree on severity: `acpe.yaml` rated site agreements a flat **Gap** (*"no pillar today models site-level compliance/contract documents"*), while `lcme.yaml` and `coda.yaml` rated the same requirement Configure with the identical caveat (`content/journeys/accreditation-self-study.yaml`, stage 3). **Corrected 2026-09-13:** that disagreement is gone. Element-level tracing retracted the ACPE rationale — a site-scoped Contract object with expiry alerts, cross-site linking and a routing checklist ships today — and 3.5.b was re-rated **Configure** on 2026-09-10, so at stage 2 the preceptor/site onboarding journey no longer carries a flat Gap element on this requirement — a later stage of that same journey still does, for unrelated elements. The caveat that survives here is narrower and common to all four accreditors: nothing confirms that a lapsed agreement *blocks* new scheduling at that site rather than only flagging it.
...
33 of 56 elements are rated Configure, and a large share of those ratings are conditional on an unconfirmed assumption: that Compliance Management can point at a site, preceptor, faculty member, or affiliated GME program rather than only a student. If that extension does not exist, those Configures degrade toward Gap. Extending a **shipped** pillar to new entity types is the cheapest, fastest move in the entire plan. On the site/contract case it is already done — ACPE 3.5.b was corrected from Gap to **Configure** on 2026-09-10 once the shipped site-scoped Contract object was verified — and what still stands between 3.5.b and Transfer is the scheduling-block behavior, the same piece COCA 6.9, LCME 1.4 and CODA 4-6 each ask for (`content/journeys/rotation-lifecycle.yaml`, stage 1).
